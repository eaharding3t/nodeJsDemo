...
function route(handle, pathname, getData, response)
{
	...
	else
	{
		...
		var htm= '<!DOCTYPE html><html><head><script>function onLoading(){var xmlHttp = new XMLHttpRequest();' 
					...
					+'};'
					+'xmlHttp.open("GET", "http://localhost:1337/onLoading?dataType=2", true);'
					+'xmlHttp.send();}'
					...
					+'};'
					+'var url = "http://localhost:1337/classExpand?subject="+sub+"&classID="+classID+"&dataType=2"; document.getElementById("sectionList").innerHTML = "pls";'
					+'xmlHttp.open("GET",url,true);'
					+'xmlHttp.send();}'
					...
					+'};'
					+'var url = "http://localhost:1337/sectionExpand?subject="+sub+"&classID="+classID+"&section="+sec+"&dataType=2";'
					+'xmlHttp.open("GET",url,true);'
					+'xmlHttp.send();}'
					+'</script></head>'
